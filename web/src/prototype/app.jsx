...
// Pull sibling-file components off the global namespace. The prototype's
// original CDN/Babel-standalone build let every file share one global scope;
// under Vite the .jsx files attach their public surface to `window` (see
// the Object.assign(window, …) calls at the bottom of markets.jsx and
// bond.jsx). Reading from window here lets the visual JSX below stay
// completely untouched.
const MarketsPage  = (props) => React.createElement(window.MarketsPage, props);
const MarketDetail = (props) => React.createElement(window.MarketDetail, props);
const BondPage     = (props) => React.createElement(window.BondPage, props);

// The dev tweaks panel lived in tweaks-panel.jsx — useful while iterating
// on the prototype but never user-facing. The shims below render nothing
// for any TweakX component so we don't have to port the panel itself.
...
function HeaderNav({ route }) {
  ...
  return (
    <header className="headernav">
      ...
      {window.WalletPill
        ? React.createElement(window.WalletPill)
        : (
...
// main.tsx mounts <App /> — no self-mount here.
export { App };
export default App;
